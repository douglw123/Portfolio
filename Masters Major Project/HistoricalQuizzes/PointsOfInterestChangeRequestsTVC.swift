//
//  PointsOfInterestChangeRequestsTVC.swift
//  HistoricalQuizzes
//
...
class PointsOfInterestChangeRequestsTVC: UITableViewController {

    let coreDataHandler = CoreDataHandler()
    var selectedPointOfInterest:PointOfInterest?
    var pointsOfInterestChangeRequests:[PointOfInterestChangeRequest]?
    
    var changeRequestToComplete:PointOfInterestChangeRequest?
...
//        pointsOfInterestChangeRequests = coreDataHandler.getAllPointsOfInterestChangeRequests()
        pointsOfInterestChangeRequests = selectedPointOfInterest?.changeRequests?.allObjects as? [PointOfInterestChangeRequest]
    ...
    func presentAreYouSureAlert(changeRequestCell:ChangeRequestTVCell) {
        ...
        self.changeRequestToComplete = changeRequestCell.changeRequest as? PointOfInterestChangeRequest
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return pointsOfInterestChangeRequests?.count ?? 0
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let selectedPointOfInterestChangeRequest = pointsOfInterestChangeRequests?[indexPath.row]
        
//        let changeRequestCustomCell = Bundle.main.loadNibNamed("ChangeRequestTVCell", owner: self, options: nil)?.first as! ChangeRequestTVCell
//        
        changeRequestCustomCell.messageLabel.text = selectedPointOfInterestChangeRequest?.message
        changeRequestCustomCell.changeRequestTVC = self
        changeRequestCustomCell.changeRequest = selectedPointOfInterestChangeRequest
        
        if (selectedPointOfInterestChangeRequest?.deleteOnNextSync)! {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let controller = segue.destination as? UINavigationController {
            if let editPointOfInterestTVC = controller.viewControllers.first as? EditPointOfInterestTVC {
